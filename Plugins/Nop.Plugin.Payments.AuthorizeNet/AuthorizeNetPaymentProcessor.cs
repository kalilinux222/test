...
namespace Nop.Plugin.Payments.AuthorizeNet
{
    ...
    public class AuthorizeNetPaymentProcessor : BasePlugin, IPaymentMethod
    {
        ...
        public AuthorizeNetPaymentProcessor(AuthorizeNetPaymentSettings authorizeNetPaymentSettings,
            ...
            IOrderTotalCalculationService orderTotalCalculationService, IEncryptionService encryptionService, ILocalizationService localizationService)
        {
            ...
            if (!String.IsNullOrEmpty(reply))
            {
                ...
                switch (responseFields[0])
                {
                    ...
                    case "2":
                        ...
                        break;
                    case "3":
                        var errorMessage = string.Format("Error: {0}", reply);

                        //https://www.authorize.net/content/dam/anet-redesign/documents/ReportingGuide_SOAP.pdf#G10.1021541
                        //8 = The credit card has expired.
                        if (responseFields.Length > 2 && responseFields[2] == "8")
                        {
                            errorMessage = _localizationService.GetResource("Plugins.Payments.AuthorizeNet.Errors.CreditCardExpired");
                        }

                        //11 = A duplicate transaction has been submitted.
                        if (responseFields.Length > 2 && responseFields[2] == "11")
                        {
                            errorMessage = _localizationService.GetResource("Plugins.Payments.AuthorizeNet.Errors.DuplicateTransaction");
                        }
