...
namespace Nop.Web.Extensions
{
    ...
    public static class ControllerExtensions
    {
            ...
            Product product)
        {
            ...
            return cacheManager.Get(cacheKey, () => GetSpecList(product, specificationAttributeService));
        }

        //y.chan - custom
        private static IList<ProductSpecificationModel> GetSpecList(Product product, ISpecificationAttributeService specificationAttributeService)
        {
            IList<ProductSpecificationModel> retList =
                specificationAttributeService.GetProductSpecificationAttributes(product.Id, 0, null, true)
                .Select(psa =>
                {
                    var m = new ProductSpecificationModel
            ...
            if (product.Length != 0)
            {
                ProductSpecificationModel psaModelLength = new ProductSpecificationModel();
                psaModelLength.SpecificationAttributeId = 1000000;
                psaModelLength.SpecificationAttributeName = "Length";
                psaModelLength.ValueRaw = Math.Round(product.Length, 2).ToString("#.##") + " inches";
                retList.Add(psaModelLength);
            }
            if (product.Width != 0)
            {
                ProductSpecificationModel psaModelWidth = new ProductSpecificationModel();
                psaModelWidth.SpecificationAttributeId = 1000001;
                psaModelWidth.SpecificationAttributeName = "Width";
                psaModelWidth.ValueRaw = Math.Round(product.Width, 2).ToString("#.##") + " inches";
                retList.Add(psaModelWidth);
            }
            if (product.Height != 0)
            {
                ProductSpecificationModel psaModelHeight = new ProductSpecificationModel();
                psaModelHeight.SpecificationAttributeId = 1000002;
                psaModelHeight.SpecificationAttributeName = "Height";
                psaModelHeight.ValueRaw = Math.Round(product.Height, 2).ToString("#.##") + " inches";
                retList.Add(psaModelHeight);
            }
            if (product.Weight != 0)
            {
                ProductSpecificationModel psaModelWeight = new ProductSpecificationModel();
                psaModelWeight.SpecificationAttributeId = 1000003;
                psaModelWeight.SpecificationAttributeName = "Weight";
                psaModelWeight.ValueRaw = Math.Round(product.Weight, 2).ToString("#.##") + " lbs";
                retList.Add(psaModelWeight);
            }
            return retList;
        }
        //y.chan - custom - end
                                    ...
                                    if (!product.CustomerEntersPrice)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            decimal minPossiblePrice = priceCalculationService.GetFinalPrice(product,
                                                workContext.CurrentCustomer, decimal.Zero, true, int.MinValue);
                ...
                if (preparePictureModel)
                {
                    ...
                    model.DefaultPictureModel = cacheManager.Get(defaultProductPictureCacheKey, () =>
                    {
                        ...
                        pictureModel.Title = (picture != null && !string.IsNullOrEmpty(picture.TitleAttribute)) ?
                            ...
                            string.Format(localizationService.GetResource("Media.Product.ImageLinkTitleFormat"), model.ShortDescription);
                        //y.chan - custom - end
                        //"alt" attribute
