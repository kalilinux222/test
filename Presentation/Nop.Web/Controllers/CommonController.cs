...
namespace Nop.Web.Controllers
{
    public partial class CommonController : BasePublicController
    {
        ...
        {
            //try loading a theme specific favicon
            var faviconFileName = "Themes/" + _themeContext.WorkingThemeName + "/Content/img/favicon.ico";
            var localFaviconPath = System.IO.Path.Combine(_webHelper.MapPath("~/"), faviconFileName);
            if (!System.IO.File.Exists(localFaviconPath))
            {
                //try loading a store specific favicon
                faviconFileName = string.Format("favicon-{0}.ico", _storeContext.CurrentStore.Id);
