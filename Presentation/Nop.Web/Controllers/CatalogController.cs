    ...
    public partial class CatalogController : BasePublicController
    {
        ...
        public ActionResult Category(int categoryId, CatalogPagingFilteringModel command)
        {
            ...
            model.SubCategories = _cacheManager.Get(subCategoriesCacheKey, () =>
                ...
                .Select(PrepareSubCategoryModel)
                .ToList()
            );

            //additional sub-categories
            var additionalSubCategoryIds = SettingService.GetSettingByKey("CatalogSettings.AdditionalSubCategories-" + categoryId, new List<int>(), _storeContext.CurrentStore.Id, true);

            if (additionalSubCategoryIds.Count > 0)
            {
                IList<CategoryModel.SubCategoryModel> additionalSubCategoryModels = new List<CategoryModel.SubCategoryModel>();

                foreach (var additionalSubCategoryId in additionalSubCategoryIds)
                {
                    var additionalSubCategory = _categoryService.GetCategoryById(additionalSubCategoryId);

                    if (additionalSubCategory != null)
                    {
                        additionalSubCategoryModels.Add(PrepareSubCategoryModel(additionalSubCategory));
                    }
                }

                if (additionalSubCategoryModels.Count > 0)
                {
                    model.CustomProperties.Add("AdditionalSubCategories", additionalSubCategoryModels);
                }
            }

            var popularCategoryIds = SettingService.GetSettingByKey("PopularCategoriesSettings.Categories", new List<int>(), _storeContext.CurrentStore.Id, true);

            if (popularCategoryIds.Contains(categoryId))
            {
                var popularCategoriesForCategoryIds = category.GetAttribute<List<int>>("PopularCategoryIds");

                if (popularCategoriesForCategoryIds.Count > 0)
                {
                    IList<CategoryModel.SubCategoryModel> popularCategoriesForCategory = new List<CategoryModel.SubCategoryModel>();

                    foreach (var popularCategoryId in popularCategoriesForCategoryIds)
                    {
                        var popularCategory = _categoryService.GetCategoryById(popularCategoryId);

                        if (popularCategory != null)
                        {
                            popularCategoriesForCategory.Add(PrepareSubCategoryModel(popularCategory));
                        }
                    }

                    if (popularCategoriesForCategory.Count > 0)
                    {
                        model.CustomProperties.Add("PopularCategories", popularCategoriesForCategory);
                    }
                }
            }
        ...
        private CategoryModel.SubCategoryModel PrepareSubCategoryModel(Category category)
        {
            ...
            var categoryPictureCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_PICTURE_MODEL_KEY, category.Id, pictureSize,
                true, _workContext.WorkingLanguage.Id, _webHelper.IsCurrentConnectionSecured(), _storeContext.CurrentStore.Id);
            ...
            subCatModel.PictureModel = _cacheManager.Get(categoryPictureCacheKey, () =>
            {
                ...
                var pictureModel = new PictureModel
                {
                    ...
                    Title = string.Format(_localizationService.GetResource("Media.Category.ImageLinkTitleFormat"),
                        subCatModel.Name),
                    AlternateText = string.Format(_localizationService.GetResource("Media.Category.ImageAlternateTextFormat"),
                        subCatModel.Name)
                };
        ...
        public ActionResult HomepageCategories()
        {
            ...
            var model = _cacheManager.Get(categoriesCacheKey, () =>
                ...
                .Select(x =>
                {
                    ...
                    var categoryDescription = x.GetAttribute<string>("CustomDescription");

                    if (!string.IsNullOrEmpty(categoryDescription))
                    {
                        catModel.CustomProperties.Add("CustomDescription", categoryDescription);
                    }
