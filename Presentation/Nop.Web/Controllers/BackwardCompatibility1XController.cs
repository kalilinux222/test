...
namespace Nop.Web.Controllers
{
    public partial class BackwardCompatibility1XController : BasePublicController
    {
                ...
                case "blog":
                    {
                        ...
                        {
                            return RedirectTagBlogPost(Request.QueryString["tag"]);
                        }

                        if (!string.IsNullOrEmpty(Request.QueryString["month"]))
                        {
                            return RedirectMonthBlogPost(Request.QueryString["month"]);
                        }

                        if (!string.IsNullOrEmpty(Request.QueryString["blogpostid"]))
                        {
                            return RedirectBlogPost(Request.QueryString["blogpostid"], false);
                        }

                        return RedirectToRoutePermanent("Blog");
                    }
                case "topic":
                    {
        ...
        public ActionResult RedirectTagBlogPost(string tag)
        {
            return RedirectToRoutePermanent("BlogByTag", new { tag });
        }

        public ActionResult RedirectMonthBlogPost(string month)
        {
            return RedirectToRoutePermanent("BlogByMonth", new { month });
        }
