...
namespace Nop.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
        ...
        protected virtual void SavePictureInFile(int pictureId, byte[] pictureBinary, string mimeType)
        {
            ...
            ////y.chan - use ImageMagick.Net to compress images - not working very well
            ////File.WriteAllBytes(GetPictureLocalPath(fileName), pictureBinary);
            //#region custom
            //string localPath = GetPictureLocalPath(fileName);
            //bool success = false;
            //#region  compress using IMageMagick.NET
            //try
            //{
            //    var localTempPath = GetPictureLocalTempPath(fileName);
            //    var io = new ImageMagick.ImageOptimizer();
            //    File.WriteAllBytes(localTempPath, pictureBinary);
            //    var pix = new FileInfo(localTempPath);

            //    io.LosslessCompress(pix);
            //    pix.Refresh();
            //    File.Copy(localTempPath, localPath);
            //    success = true;
            //}
            //catch (Exception exc)
            //{
            //    _logger.Error("Error compressing image with ImageMagick using original instead", exc);
            //}
            //#endregion

            //if (!success)
            //{
            //    //failed for some reason, do the original approach
            //    File.WriteAllBytes(localPath, pictureBinary);
            //}
            //#endregion
            ////y.chan - end
        }
        ...
        protected virtual void SaveThumb(string thumbFilePath, string thumbFileName, byte[] binary)
        {
            ...
            ////y.chan - use ImageMagick.Net to compress images - not doing good compression - remove
            ////File.WriteAllBytes(thumbFilePath, binary);
            //#region custom
            //bool success = false;
            //#region  compress using IMageMagick.NET
            //try
            //{
            //    var localTempPath = GetPictureLocalTempPath(thumbFileName);
            //    var io = new ImageMagick.ImageOptimizer();
            //    File.WriteAllBytes(localTempPath, binary);
            //    var pix = new FileInfo(localTempPath);

            //    io.LosslessCompress(pix);
            //    pix.Refresh();
            //    File.Copy(localTempPath, thumbFilePath);
            //    success = true;
            //}
            //catch (Exception exc)
            //{
        ...
        //y.chan - custom for image compression
        #region Custom
        protected virtual string GetPictureLocalTempPath(string fileName)
        {
            string folder = _webHelper.MapPath("~/content/images/optomized/");
