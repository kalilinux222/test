    ...
    public partial class ExportManager : IExportManager
    {
        ...
        protected virtual void WriteCategories(XmlWriter xmlWriter, int parentCategoryId)
        {
            ...
            if (categories != null && categories.Count > 0)
            {
                foreach (var category in categories)
                {
                    ...
                    /// Added by Kevin
                    xmlWriter.WriteElementString("Description2", null, category.Description2);
                    /// End Added by Kevin
                    /// Added by y.chan
                    xmlWriter.WriteElementString("Header1", null, category.Header1);
                    /// End Added by y.chan
        ...
        public virtual string ExportProductsToXml(IList<Product> products)
        {
            ...
            foreach (var product in products)
            {
				...
				/// Added by Kevin
                xmlWriter.WriteElementString("Ship1", null, product.Ship1.ToString());
                xmlWriter.WriteElementString("Ship10", null, product.Ship10.ToString());
				/// End Added by Kevin
        ...
        public virtual void ExportProductsToXlsx(Stream stream, IList<Product> products)
        {
            ...
            using (var xlPackage = new ExcelPackage(stream))
            {
                ...
                foreach (var p in products)
                {
					...
					/// Added by Kevin
                    worksheet.Cells[row, col].Value = p.Ship1.ToString();
                    col++;

                    worksheet.Cells[row, col].Value = p.Ship10.ToString();
                    col++;
					/// Added by Kevin
