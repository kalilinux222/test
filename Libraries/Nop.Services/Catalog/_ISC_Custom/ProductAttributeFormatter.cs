...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductAttributeFormatter : IProductAttributeFormatter
    {
        ...
        public virtual string FormatAttributes(Product product, string attributesXml, bool returnTextPrompt)
        {
            var customer = _workContext.CurrentCustomer;
            return FormatAttributes(product, attributesXml, customer, returnTextPrompt);
        }
                    ...
                    for (int j = 0; j < valuesStr.Count; j++)
                    {
                        ...
                        if (!attribute.ShouldHaveValues())
                        {
                            ...
                            if (attribute.AttributeControlType == AttributeControlType.MultilineTextbox)
                            {
                                ...
                                if (returnTextPrompt)
                                {
                                    attributeName = attribute.GetLocalized(a => a.TextPrompt, _workContext.WorkingLanguage.Id).Replace(":","");
                                }
                                else {
                                    attributeName = attribute.ProductAttribute.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id);
                                }
                                //encode (if required)
                                if (htmlEncode)
                            ...
                            else
                            {
                                ...
                                if (returnTextPrompt)
                                {
                                    formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.TextPrompt, _workContext.WorkingLanguage.Id).Replace(":", ""), valueStr);
                                }
                                else
                                {
                        ...
                        else
                        {
                            ...
                            if (int.TryParse(valueStr, out attributeValueId))
                            {
                                ...
                                if (attributeValue != null)
                                {
                                    if (returnTextPrompt)
                                    {
                                        formattedAttribute = string.Format("{0}: {1}", attribute.GetLocalized(a => a.TextPrompt, _workContext.WorkingLanguage.Id).Replace(":", ""), attributeValue.GetLocalized(a => a.Name, _workContext.WorkingLanguage.Id));
                                    }
                                    else
                                    {
