...
namespace Nop.Services.Messages
{
    public partial class MessageTokenProvider : IMessageTokenProvider
    {
        ...
        public virtual void AddOrderTokens(IList<Token> tokens, Order order, int languageId, int vendorId = 0)
        {
            ...
            var paymentMethodType = new StringBuilder();
            ...
            if (customValues != null)
            {
                foreach (var item in customValues)
                {
                    ...
                    if (item.Key == "Payment Method Type")
                    {
                        paymentMethodType.AppendFormat("{0}: {1}", HttpUtility.HtmlEncode(item.Key), HttpUtility.HtmlEncode(item.Value != null ? item.Value.ToString() : ""));
                    }
                }
            }
            tokens.Add(new Token("Order.CustomValues", sbCustomValues.ToString(), true));
            tokens.Add(new Token("Order.PaymentMethodType", paymentMethodType.ToString(), true));
