...
* ---------- -------------	------  ----------------------------------------- 
* 05/23/2017 y. Chan				Added Manufacturer Part Number logic
*									and disabled the Full Description search
* ---------- -------------	------  ----------------------------------------- 
* 06/07/2018 Nop-Templates			Products with the same display order are now
*									sorted by Price ASC
* ---------- -------------	------  ----------------------------------------- 
* 06/07/2018 Nop-Templates			Products can be searched by Manufacturer 
*									Name now.
* ---------- -------------	------  ----------------------------------------- 
* 01/19/2021 Nop-Templates			Added search by Product Title and
*                                   Product Tags from one field using
*									FullText Search.
* ---------- -------------	------  ----------------------------------------- 
* 05/26/2021 Nop-Templates			Added Search by Product Template					
******************************************************************************/
...
BEGIN
	...
	BEGIN
		...
		BEGIN		
			...
			SELECT pst.[ProductId]
			FROM [SS_FL_ProductSearchTerms] pst with (NOLOCK)
			WHERE '
			SET @sql = @sql + ' CONTAINS(pst.[SearchTerm], @Keywords) '
		END
		ELSE
		BEGIN			
			--product name
		...
		--		SELECT p.Id 
		--			FROM Product p with (NOLOCK) 
		--			WHERE p.[ManufacturerPartNumber] = @Keywords ' 
		--END 
	...
	BEGIN
		...
		IF @CategoryIdsCount > 0 SET @sql_orderby = ' pcm.DisplayOrder ASC, p.Price ASC'
